...
public:
    ...
    int find_free_block();
    std::vector<int> find_disk_path(std::string dirpath); //fetch directory paths from path without filename
    std::string path_name(std::string path); //fetch the filename from path
    std::string access_rights_to_string(uint8_t accessrights);
};
